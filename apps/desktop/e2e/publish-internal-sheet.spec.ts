  ...
  test(`inline Sheet check and source controls at ${width}px`, async ({ page }) => {
    ...
    await page.addInitScript(() => {
      ...
      w.__TAURI_INTERNALS__.invoke = async (command, args) => {
        ...
        if (command === "publish_sheet_check" || command === "publish_sheet_prepare") {
    ...
    expect(await page.evaluate(() => (window as unknown as {sheetCalls:{command:string}[]}).sheetCalls.map(c=>c.command))).toEqual(["publish_sheet_check","publish_sheet_prepare","publish_sheet_prepare"]);
