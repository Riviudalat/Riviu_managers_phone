...
export function interactionDraftFromProfile(config: JsonValue, actors: string[]): InteractionDraft {
  ...
  return {
    ...
    textSource: request.scriptedConversation ? "script" : manual.length ? "manual" : "ai",
    conversationJson: request.scriptedConversation ? JSON.stringify(request.scriptedConversation) : "",
