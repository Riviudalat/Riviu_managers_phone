...
import { deviceAppCandidates, deviceAppSelect, deviceThreadsBuild } from "../api";
...
it("reads the installed Threads version for this device", async () => {
  vi.mocked(deviceThreadsBuild).mockResolvedValue({ packageName: "com.instagram.barcelona", version: "410.0.0", locale: "vi-VN" });
  render(<DeviceDetailsDrawer device={device} machineLabel="Máy 24" currentOwner={null} ownerReadFailed={false} onClose={vi.fn()}/>);
  await userEvent.click(screen.getByRole("button", { name: "Kiểm tra phiên bản" }));
  await waitFor(() => expect(deviceThreadsBuild).toHaveBeenCalledWith("dual-phone"));
