...
use super::progress::{record_progress, PublishProgress};
use super::*;

type BeforePublish<'a> = dyn FnMut(
        Option<&riviu_core::SoundSelectionEvidence>,
        Option<&riviu_core::publish_submission::PublishSubmissionProof>,
    ) -> anyhow::Result<()>
    + Send
    + 'a;
...
pub(super) async fn post_one_phone(
    ...
    let attempt = post_one_assignment_owned(
        &control,
        &db,
        frames.as_ref(),
        &campaign_id,
    ...
    if attempt.claim_refused {
        ...
        return Err(PhoneFailure::NothingPublished(match attempt.outcome {
            ...
            PostOutcome::Posted(_) | PostOutcome::Submitted(_) => format!(
    ...
    let (mut state, mut code) = state_for_outcome(&outcome);
    let (message, mut evidence) = match &outcome {
        PostOutcome::Posted(evidence) => (None, evidence.to_string()),
        PostOutcome::Submitted(evidence) => (
            Some("Đã gửi bài tới TikTok; đang chờ xác minh liên kết, giữ TikTok chạy".into()),
            evidence.to_string(),
        ),
    ...
    if matches!(outcome, PostOutcome::Submitted(_) | PostOutcome::Unknown(_)) {
        evidence = db
            ...
            .map_err(|error| PhoneFailure::MayBeLive(error.to_string()))?;
    }
    // Pixel / unmeasured routes can Post without recording the signed-in account. Link
    // recovery then fails forever — fail closed to needsReview instead of verifying forever.
    if matches!(outcome, PostOutcome::Submitted(_)) {
        if let Ok(Some(detail)) = db.get_publish_campaign(&campaign_id) {
            if let Some(row) = detail
                .assignments
                .iter()
                .find(|row| row.id == assignment.id)
            {
                if publish_expected_account_missing(row.effect_intent.as_deref()) {
                    evidence = stamp_submission_identity_review(&evidence);
                    state = riviu_core::PublishCampaignState::Uncertain;
                    code = Some("post_verification_needs_review");
                }
            }
        }
    }
    ...
    let written = if let Some(run) = run.as_ref() {
        db.settle_pipeline_post(
            run,
            &assignment.id,
            attempt.final_revision.expect("owned attempt revision"),
            state,
            code,
            &evidence,
            owed.as_deref(),
        ...
        .and_then(|changed| {
            anyhow::ensure!(changed, "stale publish worker result");
            Ok(())
        })
    } else {
        match owed {
    ...
    record_progress(
        ...
        &assignment.id,
        match &outcome {
            PostOutcome::Posted(_) => PublishProgress::Finished,
            PostOutcome::Submitted(_) => PublishProgress::LinkPending {
                reason: "TikTok chưa trả liên kết đúng bài; tiếp tục xác minh, không đăng lại"
                    .into(),
            },
            PostOutcome::NothingPublished(reason) => PublishProgress::FailedBeforePost {
                reason: reason.clone(),
            },
            PostOutcome::Unknown(reason) => PublishProgress::PostUncertain {
                reason: reason.clone(),
            },
        },
    );
    ...
    match (message, &outcome) {
        ...
        // the next observation. Do not turn ordinary upload latency into a campaign error.
        (Some(_), PostOutcome::Submitted(_)) => Ok(()),
...
pub(super) fn stored_campaign_input_digest(
    ...
    riviu_core::db::publish_campaign_input_digest(request, detail)
}
...
pub(crate) async fn transfer_publish_campaign_inner(
    ...
    for assignment in &detail.assignments {
        ...
        let staged = match stage_one_bundle(bundle, assignment.ordinal) {
            Ok(staged) => staged,
            Err(error) => {
                ...
                db.update_publish_assignment_state(
                    ...
                    Some("media_staging_failed"),
                    Some(&serde_json::json!({"message":message}).to_string()),
                )?;
                db.settle_publish_transfer(
                    &campaign_id,
                    riviu_core::db::PublishTransferSettle::FailedBeforeDispatch {
                        error_code: "media_staging_failed",
                    },
                )?;
                return Err(error.context("chuẩn bị bản chuyển media trước khi chạm thiết bị"));
            }
        };
        ...
        record_progress(
            ...
            &campaign_id,
            &assignment.id,
            PublishProgress::CheckingDevice,
        );
        ...
        record_progress(
            ...
            &campaign_id,
            &assignment.id,
            PublishProgress::DeviceReady,
        );
        record_progress(
            &db,
            &campaign_id,
            &assignment.id,
            PublishProgress::TransferringMedia {
                count: if bundle.video.is_some() {
                    1
                } else {
                    bundle.images.len()
                },
                video: bundle.video.is_some(),
            },
        );
        match control
                ...
                match native_result {
                    Ok(evidence) => {
                        ...
                        record_progress(
                            ...
                            &campaign_id,
                            &assignment.id,
                            PublishProgress::MediaTransferred,
                        );
                    }
...
pub(crate) async fn execute_publish_campaign_inner(
    ...
    let mut detail = db
        ...
        .ok_or_else(|| anyhow::anyhow!("publish campaign not found"))?;
    anyhow::ensure!(
        !db.has_active_publish_pipeline(&campaign_id)?,
        "chiến dịch đang chạy; theo dõi tiến độ từng máy"
    );
    let existing_snapshot = db.get_publish_execution_snapshot(&campaign_id)?;
    let input_digest = existing_snapshot
        .as_ref()
        .map(|snapshot| snapshot.input_digest.clone())
        .unwrap_or(stored_campaign_input_digest(&request, &detail)?);
    ...
    let has_fresh_assignment = fresh_assignments_allowed_by_retry_scope(
        existing_snapshot
            .as_ref()
            .map(|snapshot| snapshot.retry_scope),
        &detail.assignments,
    );
    ...
    if has_fresh_assignment && issues.is_empty() {
        if let Err(error) = super::pipeline::execute_pipeline(
            control.clone(),
            db.clone(),
            frames.clone(),
            events.clone(),
        ...
        .await
        {
            if error.is::<super::pipeline::PipelineClaimRejected>() {
                return Err(error);
            }
    ...
    // Verification can advance only a previously dispatched assignment and carries its
    // revision into settlement. It never reuses the fresh composer/transfer branch.
    detail = db
        .get_publish_campaign(&campaign_id)?
        .context("campaign missing before verification")?;
    let pending = db.publish_verifications_for_campaign(&campaign_id, 1000)?;
    for candidate in pending
        .iter()
        .filter(|candidate| candidate.campaign_id == campaign_id)
    {
        let due =
            !has_fresh_assignment || !candidate.scheduled || candidate.is_due(chrono::Utc::now());
        let free = control.current_work_owner(&candidate.udid).is_none();
        ...
        if !due {
            issues.push(publish_issue(
                "link_check_not_due",
                assignment,
                "Chưa tới lịch kiểm tra liên kết cho máy này; giữ máy rảnh và thử lại sau.",
            ));
            continue;
        }
        if !free {
            issues.push(publish_issue(
                "link_check_device_busy",
                assignment,
                "Máy đang bận việc khác; không chiếm quyền để kiểm tra liên kết.",
            ));
            continue;
        }
        match super::verification::verify_pending_assignment(&control, &db, &events, candidate)
            .await
        {
            Ok(_) => {}
            Err(error) => {
    ...
    detail = db
        .get_publish_campaign(&campaign_id)?
        .context("campaign missing after verification")?;
        ...
        let resume = match assignment.state {
            ...
            riviu_core::PublishCampaignState::Verifying => {
                issues.push(publish_issue(
                    ...
                    Some(&assignment),
                    "TikTok đã nhận thao tác Đăng; đang chờ xác minh bài và lấy liên kết",
                ));
                continue;
            }
            riviu_core::PublishCampaignState::Uncertain
                if assignment.error_code.as_deref() == Some("post_verification_needs_review") =>
            {
                ambiguous = true;
                let reason = current_evidence
                    .as_ref()
                    .and_then(|value| value["verificationStatus"]["reason"].as_str())
                    .unwrap_or("Hết thời gian chờ liên kết; cần kiểm tra bài trên máy");
                issues.push(publish_issue(
...
impl riviu_core::PublishRuntimePort for DesktopPublishRuntimePort {
    ...
    async fn capture_canonical_link(
        ...
        let mut evidence = evidence_with_post_url(self.current_evidence.clone(), &link);
        evidence["verificationDiagnostic"] = diagnostic;
        self.db
    ...
    async fn write_sheet(
        ...
        self.current_evidence = Some(evidence);
        self.db
            .retry_bound_sheet_assignment(assignment_id, chrono::Utc::now().timestamp_millis())
            .map_err(|error| error.to_string())?;
        deliver_assignment_sheet_row(&self.db, &self.events, assignment_id).await?;
        match self
            .db
            .publish_sheet_outbox_state(assignment_id)
            .map_err(|error| error.to_string())?
        {
            Some(riviu_core::db::SheetOutboxState::Sent) => Ok(()),
            _ => Err("Sheet chưa xác nhận; hàng đang chờ hoặc do worker khác xử lý".into()),
        }
    }
...
pub(super) fn campaign_retry_scope(
    ...
    if issues
        .iter()
        .any(|issue| issue.code == "post_verification_needs_review")
    ...
    if status != riviu_core::PublishExecutionStatus::Uncertain
        && issues
            .iter()
            .any(|issue| issue.code == "post_verification_pending")
    {
        return riviu_core::PublishRetryScope::LinkAndSheet;
    }
    if status == riviu_core::PublishExecutionStatus::Uncertain
...
/// The persisted retry scope is the operator's reviewed boundary. A mixed campaign
/// may contain an untouched sibling beside a submitted post; link/Sheet recovery
/// must never promote that sibling into a new Post in the same invocation.
pub(super) fn fresh_assignments_allowed_by_retry_scope(
    retry_scope: Option<riviu_core::PublishRetryScope>,
    assignments: &[riviu_core::PublishAssignmentRecord],
) -> bool {
    if assignments
        .iter()
        .any(|row| row.error_code.as_deref() == Some("post_verification_needs_review"))
    {
        return false;
    }
    if matches!(
        retry_scope,
        Some(
            riviu_core::PublishRetryScope::LinkAndSheet | riviu_core::PublishRetryScope::SheetOnly
        )
    ) {
        return false;
    }
...
fn publish_expected_account_missing(intent: Option<&str>) -> bool {
    intent
        .and_then(|raw| serde_json::from_str::<serde_json::Value>(raw).ok())
        .map(|value| {
...
fn stamp_submission_identity_review(evidence_json: &str) -> String {
    ...
    evidence["verificationStatus"] = serde_json::json!({
        "state": "needsReview",
        "reason": "Thiếu bằng chứng tài khoản hoặc thời điểm của lần Đăng; cần kiểm tra bài trên máy, không đăng lại",
        "cause": "submissionIdentityMissing",
        "attempts": evidence["verificationStatus"]["attempts"].as_u64().unwrap_or(0),
        "readFailures": 0,
        "nextCheckAt": serde_json::Value::Null,
    });
    evidence.to_string()
}
...
pub(super) fn missing_link_locators(
    ...
    let Some(labels) = riviu_core::tiktok_labels::controls_for_runtime(package, locale, version)
    else {
...
pub(super) struct ConfirmedAssignmentLink {
    pub url: String,
...
pub(super) async fn capture_confirmed_assignment_link(
    ...
    anyhow::ensure!(
        ...
        "link recovery requires a submitted post; never dispatch Post here"
    );
    anyhow::ensure!(
    ...
    let outcome = async {
        ...
        session.set_gui_scope(riviu_core::ui_automation::GuiScope {
            run_id: assignment.campaign_id.clone(),
            assignment_id: Some(assignment.id.clone()),
            device_id: assignment.udid.clone(),
            deadline_ms: None,
        });
        ...
        let language = session
            .ui_language()
            .await
            .context("UI language unavailable")?;
        let version = session
            .app_version(&package)
            .await
            .context("app version unavailable")?;
        anyhow::ensure!(
            missing_link_locators(&package, &language, &version).is_empty(),
            "link locators missing for this build"
        );
    ...
    match (outcome, closed) {
        ...
        (Ok(_), Err(error)) => Err(anyhow::anyhow!(
            "link captured but context release failed: {error}"
        )),
    }
}
...
pub(super) enum PostOutcome {
    /// A canonical link was captured from the caption-identified own post.
    Posted(serde_json::Value),
    /// Post was submitted, but publication has not yet been verified. Never retry Post.
...
#[allow(clippy::too_many_arguments)]
async fn post_one_assignment_owned(
    control: &DeviceControlPlane,
    db: &Database,
    frames: &dyn FrameSource,
    campaign_id: &str,
    ...
    let progress = |step| record_progress(db, campaign_id, &assignment.id, step);
    progress(PublishProgress::OpeningApp);
    let context = match open_publish_context_until(control, &assignment.udid, || {
        run.is_some_and(|r| !db.publish_pipeline_current(r).unwrap_or(false))
            || db
                .publish_campaign_state(campaign_id)
                .map(|s| matches!(s, Some(riviu_core::PublishCampaignState::Cancelled)))
                .unwrap_or(true)
    })
    .await
    {
    ...
    progress(PublishProgress::AppReady);
        ...
        >|
         -> anyhow::Result<()> {
            if cfg!(debug_assertions)
                && std::env::var("RIVIU_PUBLISH_REHEARSAL").as_deref() == Ok("1")
                && db.get_setting("publish.rehearsal")?.as_deref() == Some("stopBeforePost")
            {
                record_progress(
                    ...
                    campaign_id,
                    &assignment.id,
                    PublishProgress::RehearsalReady,
                );
                anyhow::bail!("rehearsal verified before public Post");
            }
            if effect_claimed {
                anyhow::bail!("effect-intent callback invoked more than once");
            }
            let proof = identity.context("Thiếu bằng chứng tài khoản và thời điểm trước Đăng")?;
            proof.validate()?;
            let dispatch_time = proof.submitted_at.clone();
            let mut intent = serde_json::to_value(proof)?;
            intent["effectIntent"] = serde_json::json!("post");
            intent["soundSelection"] = serde_json::to_value(sound_selection)?;
            let intent = intent.to_string();
            let claim = match run {
                Some(run) => db.claim_pipeline_post(
                    run,
                    &assignment.id,
                    initial_revision.expect("owned revision"),
                    &intent,
                ),
                None => db.claim_publish_assignment_for_posting(&assignment.id, &intent),
            };
            match claim {
                Ok(true) => {
                    effect_claimed = true;
                    submitted_at = Some(dispatch_time);
                    Ok(())
                }
                Ok(false) => {
                    claim_refused = true;
                ...
                Err(error) => {
                    // Validation rolled back the claim. Keep the operator's cause only if
                    // this worker still owns the unsubmitted revision; stale workers lose.
                    if let Some(run) = run {
                        let evidence = serde_json::json!({"message":error.to_string(),"priorEvidenceJson":assignment.evidence_json});
                        let _ = db.settle_pipeline_post(
                            run,
                            &assignment.id,
                            initial_revision.expect("owned revision"),
                            riviu_core::PublishCampaignState::FailedBeforeDispatch,
                            Some("submission_proof_invalid"),
                            &evidence.to_string(),
                            None,
                            poster_identity(),
                            &bundle.partners,
    ...
    if let (Some(submitted_at), PostOutcome::Posted(evidence) | PostOutcome::Submitted(evidence)) =
        (submitted_at, &mut action_result)
    {
        evidence["submittedAt"] = serde_json::Value::String(submitted_at);
    }
    // Always release control ownership, but leave TikTok and the imported media alive
    // after an unverified Post. Returning to the feed occurs while uploads still run.
    progress(if must_preserve_pending_upload(&action_result) {
        PublishProgress::ReleasingPendingUpload
    } else {
        PublishProgress::Finishing
    });
    let rehearsal_cleanup = cfg!(debug_assertions)
        && std::env::var("RIVIU_PUBLISH_REHEARSAL").as_deref() == Ok("1")
        && campaign_id.starts_with("bench-")
        && !effect_claimed
        && db
            .get_setting("publish.rehearsal")
            .ok()
            .flatten()
            .as_deref()
            == Some("stopBeforePost");
    let cleanup = if must_preserve_pending_upload(&action_result) {
        release_pending_publish_context(control, context, &import).await
    } else if rehearsal_cleanup {
        tidy_up_the_imported_media(control, context, &assignment.udid, &import).await
    } else {
        finish_import_with_policy(
            control,
            context,
            &assignment.udid,
            &import,
...
pub(super) fn fold_cleanup_into(
    ...
    match outcome {
        ...
        PostOutcome::Submitted(evidence) => {
            PostOutcome::Submitted(serde_json::json!({"post": evidence, "cleanup": note}))
        }
...
/// An attempted Post remains an upload owner until own-post link verification succeeds.
fn must_preserve_pending_upload(outcome: &PostOutcome) -> bool {
    matches!(outcome, PostOutcome::Submitted(_) | PostOutcome::Unknown(_))
}

async fn release_pending_publish_context(
    control: &DeviceControlPlane,
    ...
    Ok(serde_json::json!({
        "state": "kept",
        "importId": import,
        "reason": "post_verification_pending",
        "appCleanup": {
            "state": "leftRunning",
            "reason": "upload_or_processing_may_still_be_active",
            "controlReleased": true,
            "udid": proof.udid,
            "stoppedStreamGeneration": proof.stopped_generation,
            "nextStreamGeneration": proof.next_generation,
        }
    }))
}

/// Close TikTok and retain evidence. Delete only this import when the campaign opted in
/// and Post was verified. Called only before Post or after canonical own-link proof.
...
pub(crate) async fn open_publish_context(
    ...
    open_publish_context_until(control, udid, || false).await
}

async fn open_publish_context_until(
    control: &DeviceControlPlane,
    udid: &str,
    ...
    let (exclusive, capacity) = control
        .reserve_ui_capacity_until(exclusive, cancelled, || {
            log::info!("publish {udid}: Chờ lượt điều khiển")
        })
        .await
        .map_err(anyhow::Error::new)?;
...
///
/// # This screen is only read on macOS; colour changes observe submission, not success
///
...
///
/// [`wait_for_post_frame`] observes a transition from two signals — the frame changed,
/// and the red of the Post button left the
...
/// blocked post remains `Submitted` and requires independent own-post verification.
///
/// What this does and does not do about that:
///
/// * The evidence names how the transition was observed (`submissionObservedBy`) and what the screening
...
pub(super) fn stage_one_bundle(
    ...
    // Scheduled siblings share a managed parent and each campaign's ordinal is zero.
    // A unique attempt directory prevents one concurrent copy/drop from deleting another.
    let root = campaign_root
        .join(".transfer")
        .join(format!("{ordinal}-{}", Uuid::new_v4()));
...
pub(super) async fn post_through_the_pixel_grid(
    ...
    let evidence = async {
        ...
        Ok::<serde_json::Value, anyhow::Error>(serde_json::json!({
            ...
            "submissionObservedBy": "frame_change_and_redness_drop",
            "publicationVerified": false,
            "linkCaptureReason": "pixel_route_requires_own_post_verification",
    ...
    match evidence {
        Ok(evidence) => PostOutcome::Submitted(evidence),
...
pub(super) async fn post_through_the_composer(
    ...
    let approved_build = db
        .publish_campaign_request(campaign_id)
        .ok()
        .flatten()
        .is_some_and(|request| {
            request.verification_builds.iter().any(|build| {
                build.udid == udid
                    && build.package == package
                    && build.version == version
                    && riviu_core::tiktok_labels::normalise_language(&build.locale)
                        == riviu_core::tiktok_labels::normalise_language(&language)
            })
        });
    if !approved_build {
        return refuse("TikTok đã đổi so với lần kiểm tra; kiểm tra lại trước Đăng".into());
    }
    let verification_plan = match riviu_core::tiktok_share::PublishVerificationPlan::for_runtime(
        &package, &language, &version,
    ) {
        Ok(plan) => plan,
        Err(error) => {
            return refuse(format!(
                "Thiếu nhận diện xác minh liên kết trước Đăng: {error}"
            ))
        }
    };
    if let Err(error) = riviu_core::tiktok_share::probe_clipboard_restore(session).await {
        return refuse(format!(
            "Helper clipboard chưa sẵn sàng trước Đăng: {error}"
        ));
    }
    let expected_account =
        match riviu_core::tiktok_share::observe_publish_account(session, &labels).await {
            Ok(account) => account,
    ...
    struct AccountReservation<'a> {
        db: &'a Database,
        assignment: &'a str,
    }
    impl Drop for AccountReservation<'_> {
        fn drop(&mut self) {
            if let Err(error) = self.db.release_unsubmitted_publish_account(self.assignment) {
                log::warn!(
                    "Chưa nhả được tài khoản của lượt {}: {error}",
                    self.assignment
                );
            }
        }
    }
    let _reservation = AccountReservation {
        db,
    ...
    let mut record_effect_intent = |selection: &riviu_core::SoundSelectionEvidence| {
        ...
        let proof = riviu_core::publish_submission::PublishSubmissionProof {
            ...
            caption_sha256: bundle.caption_sha256.clone(),
            bundle_id: bundle.id.clone(),
            media_kind: bundle.media_kind.clone(),
        };
        before_post(Some(selection), Some(&proof))?;
    ...
    let mut evidence = serde_json::json!({
        ...
        "pickerImageCountConfirmed": verdict.is_submitted()
    ...
    match verdict {
        ComposerVerdict::Submitted => {
            ...
            //
            // Submission is not publication. Only this caption-identified route followed
            // by a canonical link authorizes success, media deletion and app termination.
            let Some(submitted_at) = submitted_at else {
                return PostOutcome::Unknown(
                    "composer returned submission without durable dispatch time".into(),
                );
            };
            let identity = riviu_core::tiktok_share::SubmissionIdentity {
            ...
            if defer_link_capture {
                return capture_or_defer_submission(
                    evidence,
                    true,
                    riviu_core::tiktok_share::capture_own_post_link_for_submission(
                        session,
                        &labels,
                        &bundle.caption,
                        &identity,
            ...
            if let PostOutcome::Submitted(ref mut value) = outcome {
                value["linkCaptureReason"] = serde_json::json!(reason);
            }
            outcome
        }
...
/// Scheduled runs finish the posting session before observing any profile/share UI.
/// The capture future stays unpolled; the durable verification worker owns that later read.
pub(super) async fn capture_or_defer_submission(
    mut evidence: serde_json::Value,
    defer: bool,
    capture: impl std::future::Future<Output = riviu_core::tiktok_share::OwnPostLink>,
    progress: &riviu_core::tiktok_composer::PublishProgressObserver<'_>,
) -> PostOutcome {
    if defer {
    ...
    progress(PublishProgress::CapturingLink);
    settle_submission_capture(evidence, capture.await, progress).await
}

/// Only the identity-aware capture may promote submission into a verified publication.
pub(super) async fn settle_submission_capture(
    mut evidence: serde_json::Value,
    capture: riviu_core::tiktok_share::OwnPostLink,
    progress: &riviu_core::tiktok_composer::PublishProgressObserver<'_>,
) -> PostOutcome {
    // Cleanup can wrap a previous attempt. Retain its fields but keep all current proof
    // in the same post object, so a nested stale URL never wins over the latest capture.
    let wrapped = evidence
        .get("post")
        .is_some_and(serde_json::Value::is_object);
    if wrapped {
        if let Some(object) = evidence.as_object_mut() {
            object.remove("postUrl");
            object.remove("postConfirmedBy");
        }
    }
    let proof = if wrapped {
        &mut evidence["post"]
    } else {
        &mut evidence
    };
    proof["linkCaptureReason"] = serde_json::Value::String(capture.reason());
    // Evidence may be carried across retry attempts; never promote using a stale field.
    if let Some(object) = proof.as_object_mut() {
        object.remove("postUrl");
        object.remove("postConfirmedBy");
    }
    proof["state"] = serde_json::Value::String("submitted".into());
    proof["publicationVerified"] = serde_json::Value::Bool(false);
    if let Some(link) = capture.link() {
        match riviu_core::tiktok_share::resolve_canonical_post_link(link).await {
            Ok(link) => proof["postUrl"] = serde_json::Value::String(link),
            Err(error) => {
                proof["linkCaptureReason"] =
                    serde_json::Value::String(format!("chuẩn hóa liên kết: {error}"))
            }
        }
    }
    if post_url_owed(proof).is_some() {
        proof["state"] = serde_json::Value::String("posted".into());
        proof["publicationVerified"] = serde_json::Value::Bool(true);
        proof["postConfirmedBy"] =
            serde_json::Value::String("canonical_link_from_caption_identified_own_post".into());
        progress(PublishProgress::PostConfirmed);
        progress(PublishProgress::LinkCaptured);
        PostOutcome::Posted(evidence)
    } else {
        progress(PublishProgress::LinkPending {
            reason: proof["linkCaptureReason"]
                .as_str()
                .unwrap_or_default()
                .into(),
        });
...
#[cfg(test)]
#[path = "execution_post_tests.rs"]
mod execution_post_tests;
