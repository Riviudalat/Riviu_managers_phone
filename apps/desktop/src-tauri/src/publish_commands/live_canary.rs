...
async fn live_publish_canary() -> anyhow::Result<()> {
    ...
    let result: anyhow::Result<()> = async {
        ...
        if mode == "rehearse" || mode == "sound-scout" {
            ...
            let outcome = post_through_the_composer(&control, &db, &assignment.id, session.as_ref(), &campaign.id, SERIAL, bundle, &import, &campaign_request.sound_policy, false, &mut before_post, &|_| {}).await;
